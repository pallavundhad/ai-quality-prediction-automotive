...
A predictive quality control platform for German automotive manufacturing. It scores each part 0–100 every 10 seconds using a Random Forest model, fires SMS/email alerts when scores drop below threshold, and shows plant managers exactly *why* a part is at risk and that too in plain German.
...
**Model:** Random Forest Classifier which is chosen for explainability (Klaus needs to understand *why* the AI flagged a part), ensemble robustness, and fast inference (<10ms per prediction) suitable for edge deployment on the factory floor.
